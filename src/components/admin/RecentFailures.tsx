        ...
        <div className="bg-card p-6 rounded-xl shadow-sm border border-border">
            ...
            <div className="space-y-4">
                {safeData.length === 0 ? (
                    ...
                    safeData.map((fail) => (
                        <div key={fail.id} className="flex items-center justify-between p-3 bg-red-50 dark:bg-red-900/20 rounded-lg">
                            <div>
                                <p className="text-sm font-bold text-foreground">{fail.user}</p>
                                <p className="text-xs text-red-600 dark:text-red-400 font-mono mt-1">{t.admin.dashboard.recent.op}: #{fail.id.slice(-4)}</p>
                            </div>
